...
pub fn bytecode_to_lamina_unary(chunk: &BytecodeChunk) -> Option<lamina::ir::Module<'static>> {
    ...
    while pc < code.len() {
        ...
        match op {
            ...
            OP_RETURN => {
                let result = stack.pop().unwrap_or(StackVal::Param);
                if pc != code.len() {
                    return None;
                }
                builder.ret(Type::Primitive(PrimitiveType::I64), to_builder_val(result));
