...
pub fn create_realm(args: &[Value], ctx: &mut BuiltinContext) -> Result<Value, super::BuiltinError> {
    let _ = (args, ctx);
...
pub fn detach_array_buffer(args: &[Value], ctx: &mut BuiltinContext) -> Result<Value, super::BuiltinError> {
    let _ = (args, ctx);
    Err(super::BuiltinError::Throw(Value::String(
